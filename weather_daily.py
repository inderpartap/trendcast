...
def weather_daily(city_id_name_df,retail_df):
	...
	for index,val_id in enumerate(station_ids):
		...
		filtered_retail_data = retail_df[retail_df['city'].str.contains(station_names[index],case=False)]
		(start_date, end_date) = get_period(filtered_retail_data, 'date')
		if(not math.isnan(val_id)):
			response = send_request(api='weather', end_point='daily', params=dict(station=int(val_id),start=start_date,end=end_date))
