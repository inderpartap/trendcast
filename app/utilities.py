...
def get_weather(dates, cityname):
	...
	response = send_request(
				api="weather",
				end_point="daily",
				params=dict(station=int(city_id),
							start=start_date,
							end=end_date),
			)
	response_df = pd.DataFrame(response)
	dates['temperature'] = response_df[['temperature_min', 'temperature_max']].mean(axis=1)
	return dates
...
def deptlevel(cityname, department):
	...
	path_without_weather = "../models/department_level/without_weather/"+cityname+'_'+department+"_model.pkl"
	path_with_weather = "../models/department_level/weather/"+cityname+'_'+department+"_model.pkl"