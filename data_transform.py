...
def create_onehot(df):
    df = df.groupby(['date', 'province', 'city', 'department']
                    ).sum().reset_index()
...
def fill_missing(df):
    ...
    result[weatherCols] = df[weatherCols].interpolate(
...
def main():
    ...
    retail_data = pd.read_csv(datapath['retail'], index_col=0).drop(
        ["category", "class", "style", "vendor"], axis=1)
    retail_data['city'] = retail_data['city'].str.lower()

    # load weather data
    weather_data = pd.read_csv(datapath['weather']).rename(
        columns={"station_name": "city"}).drop("station_id", axis=1)
